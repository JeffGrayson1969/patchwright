...
def _run_to_done(root: Path) -> tuple[str, Journal, ArtifactStore]:
...
def test_replay_produces_deep_equal_case(tmp_path: Path) -> None:
    _, journal, store = _run_to_done(tmp_path)
...
def test_rerunning_drive_appends_no_new_entries(tmp_path: Path) -> None:
    case_id, journal, _ = _run_to_done(tmp_path)
